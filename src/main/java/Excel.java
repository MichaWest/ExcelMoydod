import Data.*;
import Data.Date;
import Data.Workers.Employee;
import Data.Workers.SchiftWorker;
import Data.Workers.Time;
import Data.Workers.Worker;
...
public class Excel {
    ...
    private final Map<Integer, SchiftWorker> schiftWorkers = new HashMap<>();
    ...
    private void putDataInCollection(){
        for(Map.Entry<DateTime, Data> action : filedata.entrySet()){
            ...
            if(schiftWorkers.containsKey(id)){
                schiftWorkers.get(id).setWorkTime(dateTime);
            }else{
                SchiftWorker sw = new SchiftWorker(id, o);
                sw.setName(data.getName());
                sw.setVorname(data.getVorname());
                sw.setWorkTime(dateTime);
                schiftWorkers.put(id, sw);
            }
    ...
    public DateTime getDateTime(Row row, int id) {
        ...
        if (turn.contains("2")&&turn.contains("Вых")) {
            Time t = new Time(month, day, hour, minute, second, (jahr-2020)%2==0 );
            dateTime.setEndEntr2(t);
    ...
    private void writeInWorbook() {
        ...
        for(Map.Entry<Integer, SchiftWorker> worker : schiftWorkers.entrySet()){
            passport(worker.getValue(), schift);
            workTime(worker.getValue(), schift);
        }
    }
